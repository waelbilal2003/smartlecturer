...
/// 🔹 صفحة عرض الملفات المحفوظة محلياً
...
class _SavedFilesPageState extends State<SavedFilesPage> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('خطأ في تحميل الملفات: $e', textDirection: TextDirection.rtl),
  ...
  Future<void> _deleteFile(SavedFileInfo fileInfo) async {
    ...
    try {
      ...
      if (success) {
        await _loadSavedFiles(); // إعادة تحميل القائمة
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('تم حذف الملف بنجاح', textDirection: TextDirection.rtl),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('خطأ في حذف الملف: $e', textDirection: TextDirection.rtl),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Future<void> _openFile(SavedFileInfo fileInfo) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('خطأ في فتح الملف: $e', textDirection: TextDirection.rtl),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Future<void> _cleanupOldFiles() async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('تنظيف الملفات القديمة', textDirection: TextDirection.rtl),
        content: const Text(
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('تم تنظيف الملفات القديمة بنجاح', textDirection: TextDirection.rtl),
            backgroundColor: Colors.green,
          ),
        );
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('خطأ في تنظيف الملفات: $e', textDirection: TextDirection.rtl),
            backgroundColor: Colors.red,
          ),
        );
      }
              ...
              const PopupMenuItem(
                ...
                child: Row(
                  children: [
                    ...
                    SizedBox(width: 8),
                    Text('تنظيف الملفات القديمة', textDirection: TextDirection.rtl),
                  ],
                ),
              ),
            ],
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  'عدد الملفات: ${_savedFiles.length}',
                  textDirection: TextDirection.rtl,
                ),
                Text(
